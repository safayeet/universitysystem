...
if ($_SESSION['role'] !== "admin" && $_SESSION['role'] !== "admission") {
 ...
 echo '<script>alert("new table name ' . $tablename . ' ")</script>';
        $sql = "INSERT INTO `universitysystem`.`studentdetails`(`id`, `password`, `name`, `location`, `contact`, `department`, `admissionyear`, `admissionsemester`, `currentsemester`, `sgpa`)
            VALUES ('$studentid', '$password', '$name', '$location', '$contact','$department', '$year', '$semester',1, 0)";
        if ($conn->query($sql) === TRUE) {
            echo '<script>alert("New record created successfully and new table name ' . $tablename . ' ")</script>';
